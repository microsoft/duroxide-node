...
const { JsSqliteProvider, JsPostgresProvider, JsClient, JsRuntime, activityTraceLog, orchestrationTraceLog, activityIsCancelled, activityGetClient, activityTag, orchestrationSetCustomStatus, orchestrationResetCustomStatus, orchestrationGetCustomStatus, orchestrationSetValue, orchestrationGetValue, orchestrationClearValue, orchestrationClearAllValues, orchestrationGetKvAllValues, orchestrationGetKvAllKeys, orchestrationGetKvLength, orchestrationPruneKvValues, initTracing } = require('../index.js');
...
class OrchestrationContext {
  ...
  scheduleActivity(name, input, options) {
    ...
    descriptor.withTag = (tag) => { descriptor.tag = tag; return descriptor; };
    return descriptor;
  }
   ...
   * @param {string} instanceId - Source orchestration instance ID
   * @param {string} key - KV key to read
   * @returns {ScheduledTask}
   */
  getValueFromInstance(instanceId, key) {
    return { type: 'getValueFromInstance', instanceId, key: String(key) };
  }
  ...
  setValue(key, value) {
    orchestrationSetValue(this.instanceId, String(key), String(value));
  }

  getValue(key) {
    return orchestrationGetValue(this.instanceId, String(key)) ?? null;
  }

  clearValue(key) {
    orchestrationClearValue(this.instanceId, String(key));
  }

  clearAllValues() {
    orchestrationClearAllValues(this.instanceId);
  }

  getKvAllValues() {
    return orchestrationGetKvAllValues(this.instanceId);
  }

  getKvAllKeys() {
    return orchestrationGetKvAllKeys(this.instanceId);
  }

  getKvLength() {
    return orchestrationGetKvLength(this.instanceId);
  }

  pruneKvValuesUpdatedBefore(cutoffMs) {
    return orchestrationPruneKvValues(this.instanceId, Math.max(0, Math.trunc(cutoffMs)));
  }
...
class PostgresProvider {
   ...
   *
   * @param {string} host - e.g. "myserver.postgres.database.azure.com"
   * @param {number} port - typically 5432
   * @param {string} database - database name
   * @param {string} user - Postgres role mapped to the Entra principal
   * @param {PostgresEntraOptions} [options]
   * @returns {Promise<PostgresProvider>}
   */
  static async connectWithEntra(host, port, database, user, options) {
    const n = await JsPostgresProvider.connectWithEntra(host, port, database, user, options ?? null);
    return new PostgresProvider(n);
  }
   ...
   * @returns {Promise<PostgresProvider>}
   */
  static async connectWithSchemaAndEntra(host, port, database, user, schema, options) {
    const n = await JsPostgresProvider.connectWithSchemaAndEntra(host, port, database, user, schema, options ?? null);
    return new PostgresProvider(n);
  }
}
...
class Client {
  ...
  async getValue(instanceId, key) {
    return await this._native.getValue(instanceId, key);
  }

  async waitForValue(instanceId, key, timeoutMs = 30000) {
    const result = await this._native.waitForValue(instanceId, key, timeoutMs);
    if (result == null) {
      throw new Error(`Timed out waiting for value '${key}' on instance '${instanceId}'`);
    }
    return result;
  }

  async getKvAllValues(instanceId) {
    return await this._native.getKvAllValues(instanceId);
  }
  ...
  async getOrchestrationStats(instanceId) {
    return await this._native.getOrchestrationStats(instanceId);
  }
...
class Runtime {
  ...
  constructor(provider, options) {
    // Pre-process workerTagFilter from user-friendly format to flat napi fields
    const nativeOptions = options ? { ...options } : undefined;
    if (nativeOptions && nativeOptions.workerTagFilter) {
      const filter = nativeOptions.workerTagFilter;
      if (typeof filter === 'string') {
        // "defaultOnly", "any", "none" — pass through as-is
      } else if (typeof filter === 'object') {
        if (filter.tags) {
          nativeOptions.workerTagFilter = 'tags';
          nativeOptions.workerTagFilterTags = filter.tags;
        } else if (filter.defaultAnd) {
          nativeOptions.workerTagFilter = 'defaultAnd';
          nativeOptions.workerTagFilterTags = filter.defaultAnd;
        }
      }
    }

    if (provider._type === 'postgres') {
      this._native = JsRuntime.fromPostgres(provider._native, nativeOptions);
    } else {
      this._native = new JsRuntime(provider._native, nativeOptions);
    }
...
class ActivityContext {
  constructor(ctxInfo) {
    ...
    this._tag = ctxInfo.tag || null;
    this._traceToken = ctxInfo._traceToken;
  }

  /**
   * Get the routing tag if this activity was scheduled with .withTag().
   * Returns null if no tag was set.
   * @returns {string|null}
   */
  tag() {
    return activityTag(this._traceToken) ?? this._tag;
  }
...
// Tag limits
const MAX_WORKER_TAGS = 5;
const MAX_TAG_NAME_BYTES = 256;
const MAX_KV_KEYS = 150;
const MAX_KV_VALUE_BYTES = 65536;
